...
窗口左上角的工具栏常驻显示当前 DSH 精确版本，并提供“检查更新”按钮；该按钮与托盘中的“检查 DSH 更新…”执行相同流程。检查或升级期间按钮会暂时禁用，并显示旋转加载状态。检查结果和升级确认使用与 DSH Web 风格一致的本地窗口，不会向 DSH 页面注入壳权限。

窗口最小化后保留在任务栏；关闭窗口会退出 App 并停止它启动的 DSH。托盘菜单仍提供版本信息、“显示主窗口”“重启 DSH”“检查 DSH 更新…”和“退出”。普通启动与重启只运行已保存的精确版本，不检查或自动升级。
...
App 始终使用同一个 npm 安装执行官方 `@deepseek-ai/dsh@<精确版本>`。新版本完成启动和页面加载后才会保存；失败时会尽力恢复原版本。npm 缓存不是持久备份，若其中没有所需版本，首次安装、升级或恢复仍需要网络。
